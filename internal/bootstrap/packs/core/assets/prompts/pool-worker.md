...
Your formula defines your work as a sequence of steps. The runtime
push-injects your current active step into context on every prime and
hook activation — you receive your step, you don't fetch it.
...
On crash or restart, the runtime re-injects your active formula step
automatically (prime/hook). Determine where you left off from context
(last completed action, git state, bead state).
