...
const (
	...
	// --duration is not specified. 45 minutes covers the worst-case Dolt
	// query latency observed on busy rigs (chatehr: 6 polecats, 50+ beads).
...
//
// Called by agents at the start of long operations (e.g., Dolt queries during
// witness patrol) to suppress idle-timeout and max-session-age timers for the
// specified duration. The existing held_until mechanism in the bead reconciler
// provides the timer-blocker semantics; this command is the agent-facing API
// for setting it without triggering a full user-hold suspend.
...
slow operations (e.g., large Dolt queries) that produce no terminal output
and would otherwise trigger a false-alarm watchdog kill.
...
The default duration (` + defaultHeartbeatDuration.String() + `) is chosen to cover the worst-case
Dolt query latency observed on busy rigs. Pass --duration to override.`,
		Args: cobra.NoArgs,
